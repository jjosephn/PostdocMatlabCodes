...
regCoeff = regress(clinicalParamsVectorTrainLabel,featureVectorTrain);
[regCoeffTrainVal,regCoeffTrainOrder] = sort(abs(regCoeff),'descend');  %# sort regression coefficients in descending order
prominentFeatTrain = featureVectorTrain(:,regCoeffTrainOrder);
prominentFeatTest = featureVectorTest(:,regCoeffTrainOrder);
...
plot(accuracyComp,'r');
axis([0 25 0 100])
title('PLSR-SVC: No. of Features vs Accuracy')
xlabel('Features')
ylabel('Accuracy(%)')
hold on;
